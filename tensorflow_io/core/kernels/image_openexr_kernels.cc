...
class OpenEXRIStream : public OPENEXR_IMF_INTERNAL_NAMESPACE::IStream {
 public:
  OpenEXRIStream(const string& filename, SizedRandomAccessFile* file,
                 int64 size)
  ...
  OPENEXR_IMF_INTERNAL_NAMESPACE::Int64 tellg() override { return pos_; }
...
class DecodeEXRInfoOp : public OpKernel {
 public:
  explicit DecodeEXRInfoOp(OpKernelConstruction* context) : OpKernel(context) {
    env_ = context->env();
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (int i = 0; i < input_file.parts(); i++) {
      ...
      for (Imf::ChannelList::ConstIterator c =
               input_file.header(i).channels().begin();
           c != input_file.header(i).channels().end(); c++) {
        switch (c.channel().type) {
          ...
          default:
            OP_REQUIRES(context, false,
                        errors::InvalidArgument("invalid data type: ",
                                                c.channel().type));
        }
        channel.back().push_back(c.name());
      }
      maxchannel = maxchannel > channel.back().size() ? maxchannel
                                                      : channel.back().size();
    }
    ...
    OP_REQUIRES_OK(context,
                   context->allocate_output(
                       0, TensorShape({static_cast<int64>(shape.size()), 2}),
                       &shape_tensor));
    ...
    OP_REQUIRES_OK(
        context,
        context->allocate_output(
            1, TensorShape({static_cast<int64>(dtype.size()), maxchannel}),
            &dtype_tensor));
    Tensor* channel_tensor = nullptr;
    OP_REQUIRES_OK(
        context,
        context->allocate_output(
            2, TensorShape({static_cast<int64>(dtype.size()), maxchannel}),
            &channel_tensor));
...
class DecodeEXROp : public OpKernel {
 public:
  explicit DecodeEXROp(OpKernelConstruction* context) : OpKernel(context) {
    env_ = context->env();
  }
  ...
  void Compute(OpKernelContext* context) override {
    ...
    for (Imf::ChannelList::ConstIterator c =
             input_part.header().channels().begin();
         c != input_part.header().channels().end(); c++) {
      if (channel == c.name()) {
        ...
        switch (pixel_type) {
          ...
          default:
            OP_REQUIRES(
                context, false,
                errors::InvalidArgument("invalid pixel type: ", pixel_type));
        }
      }
    }
    OP_REQUIRES(context, (dtype != DT_INVALID),
                errors::InvalidArgument("unable to find channel: ", channel));

    Tensor* output_tensor = nullptr;
    OP_REQUIRES_OK(context,
                   context->allocate_output(0, TensorShape({height, width}),
                                            &output_tensor));
    ...
    switch (dtype) {
      ...
      default:
        OP_REQUIRES(context, false,
                    errors::InvalidArgument("invalid data type: ", dtype));
    }

    Imf::FrameBuffer frame_buffer;
    frame_buffer.insert(channel, Imf::Slice::Make(pixel_type, base, dw, 0, 0,
...
REGISTER_KERNEL_BUILDER(Name("IO>DecodeExrInfo").Device(DEVICE_CPU),
                        DecodeEXRInfoOp);
REGISTER_KERNEL_BUILDER(Name("IO>DecodeExr").Device(DEVICE_CPU), DecodeEXROp);
