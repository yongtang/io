...
class AudioInput: public FileInput<video::AudioReader> {
 public:
  Status ReadRecord(io::InputStreamInterface* s, IteratorContext* ctx, std::unique_ptr<video::AudioReader>& state, int64 record_to_read, int64* record_read, std::vector<Tensor>* out_tensors) const override {
    if (state.get() == nullptr) {
      VideoReaderInit();
...
REGISTER_KERNEL_BUILDER(Name("AudioDataset").Device(DEVICE_CPU),
                        FileInputDatasetOp<AudioInput, video::AudioReader>);
